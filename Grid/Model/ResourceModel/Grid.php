...
namespace MageINIC\Grid\Model\ResourceModel;
...
class Grid extends AbstractDb
{
    ...
    public function load(AbstractModel $object, $value, $field = null): Grid
    {
    ...
    protected function _getLoadSelect($field, $value, $object): Select
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['grid_store' => $this->getTable('grid_store')],
                $this->getMainTable() . '.' . $linkField . ' = grid_store.' . $linkField,
                []
            )
                ->where('is_active = ?', 1)
                ->where('grid_store.store_id IN (?)', $storeIds)
                ->order('grid_store.store_id DESC')
                ->limit(1);
        }
    ...
    public function getConnection(): AdapterInterface
    {
        return $this->resourceConnection->getConnectionByName(
            $this->metadataPool->getMetadata(GridInterface::class)->getEntityConnectionName()
        );
    }
    ...
    public function lookupStoreIds(int $id): array
    {
        ...
        $select = $connection->select()
            ->from(['cps' => $this->getTable('grid_store')], 'store_id')
            ->join(
                ['cp' => $this->getMainTable()],
                'cps.' . $linkField . ' = cp.' . $linkField,
                []
            )
            ->where('cp.' . $entityMetadata->getIdentifierField() . ' = :id');
     ...
     * @return $this
     */
    public function setStore(StoreInterface $store): Grid
    {
        $this->_store = $store;
        return $this;
    }
     ...
     * @throws Exception
     */
    public function save(AbstractModel $object): Grid
    {
     ...
     * @throws Exception
     */
    public function delete(AbstractModel $object): Grid
    {
